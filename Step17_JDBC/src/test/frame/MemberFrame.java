...
public class MemberFrame extends JFrame 
   ...
   public void propertyChange(PropertyChangeEvent evt) {
      ...
      //만일 property name 이 tableCellEditor와 같고 table 이 수정중이 아니면
      if(evt.getPropertyName().equals("tableCellEditor") && !table.isEditing()) {
    	  //현재 선택된 row의 정보를 읽어와서 수정반영한
    	  int selected =table.getSelectedRow();
    	  int num=(int)model.getValueAt(selected, 0);
    	  String name=(String)model.getValueAt(selected, 1);
    	  String addr=(String)model.getValueAt(selected, 2);
